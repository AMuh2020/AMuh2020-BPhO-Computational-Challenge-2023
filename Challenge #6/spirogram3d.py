...
for planet in solar_system[1:]:
    ...
    y_array = np.array([])
    z_array = np.array([])
    for j in range(0, 360*100, int(0.36*100)):
        i = j / 100
    ...
    planet_patches.append(planet_plot)
...
time_interval = N * 10 * outermost_planet_period / (1234)
...
def update_animation(frame):
    ...
    for i in range(1, len(solar_system)):
        ...
        x_prime = x * np.cos(planet["inclination"] * np.pi / 180)
        z_prime = x * np.sin(planet["inclination"] * np.pi / 180) 
        # update the planet's position plot # print(i, planet["name"], x, y,z)
        planet_plot = planet_patches[i-1][0]
        # print(planet_plot)
        x_line.append(x_prime)
        y_line.append(y)
        z_line.append(z_prime)
...
animation = FuncAnimation(fig, update_animation,frames=f, interval=10, blit=True, repeat=True)


# show the animation
# plt.show()

# save as png after animation is done
animation.save("solar_system.png", writer="Pillow", fps=30)
